...
def pt_to_trt(pt_model_path, trt_engine_path, hyperparam_config, fp16_mode=False, workspace_size_mb=256):
    # Load PyTorch model
    model = TCNModel(hyperparam_config).eval()
    state_dict = torch.load(pt_model_path, map_location="cpu", weights_only=False)
    model.load_state_dict(state_dict)
    # Keep export on CPU to avoid Jetson CUDA OOM during conversion.
    model = model.cpu()
    ...
    if TORCHSUMMARY_AVAILABLE:
        ...
        try:
            summary(
                model,
                input_size=(hyperparam_config['input_size'], hyperparam_config['window_size']),
                device='cpu'
            )
    ...
    # Build TensorRT engine
    config = builder.create_builder_config()
    # Jetson often fails with large contiguous allocations; use a smaller workspace.
    config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, int(workspace_size_mb) << 20)
...
# subject = 'AB11_Ryan'
# pt_model = f'/home/metamobility2/Jimin/Trained Models IMUonly_fixed/OpenSim/DEP/OpenSim_{subject}_DEP/OpenSim_{subject}_DEP.pt'
...
pt_to_trt(pt_model, trt_engine, hyperparam_config, fp16_mode=True, workspace_size_mb=256)
