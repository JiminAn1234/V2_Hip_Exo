...
def init_data_buffers(n_samples):
    f1 = lambda: np.full((n_samples,), np.nan, dtype=np.float32)
    f6 = lambda: np.full((n_samples, 6), np.nan, dtype=np.float32)
    return {
        "timestamp": f1(),
        "mtr_cmd_L": f1(), "mtr_cmd_R": f1(), "mtr_pos_L": f1(), "mtr_pos_R": f1(), "mtr_vel_L": f1(), "mtr_vel_R": f1(),
        ...
        "applied_torque_L": f1(), "applied_torque_R": f1(),
        "actual_torque_L": f1(), "actual_torque_R": f1(),
        "gpio_output": f1()
    }

# data to be saved (allocated in main after control frequency is known)
data_to_save = {}
logged_samples = 0
...
def save_data(start_rec_sec=0, trial_time_sec=None, valid_len=None):
    global data_to_save, logged_samples
    ...
    # Ring-buffer reorder: recover chronological order for the most recent window.
...
def exit_signal_handler(sig, frame):
    global inference_process, input_q, logged_samples
...
def main():
    ...
    global data_to_save, Exo, logged_samples
    ...
    max_samples = int(collect_last_sec * Exo.control_freq_Hz)
    ...
    while True:
        ...
        idx = logged_samples % max_samples
        ...
        if filtered_torque_arr[1, -1] > 18: motor_cmd_val_L = 18
        elif filtered_torque_arr[1, -1] < -18: motor_cmd_val_L = -18
        else: motor_cmd_val_L = filtered_torque_arr[1, -1]
        
        if filtered_torque_arr[0, -1] > 18: motor_cmd_val_R = 18
        elif filtered_torque_arr[0, -1] < -18: motor_cmd_val_R = -18
        else: motor_cmd_val_R = filtered_torque_arr[0, -1]

        if exo_ON == False: motor_cmd_val_L, motor_cmd_val_R = 0.0, 0.0 # Exo off condition
